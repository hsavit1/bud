...
{
    //self.contentView.frame = CGRectMake(0, 0, self.bounds.size.width, 70);
    message = message_;
    imageUser.layer.cornerRadius = imageUser.frame.size.width/2;
    imageUser.layer.masksToBounds = YES;
    PFUser *lastUser = message[PF_MESSAGES_LASTUSER]; //@"lastUser"
    [imageUser setFile:lastUser[PF_USER_PICTURE]]; //@"picture"	
    [imageUser loadInBackground];
    labelDescription.text = message[PF_MESSAGES_DESCRIPTION]; //@"description"
    labelLastMessage.text = message[PF_MESSAGES_LASTMESSAGE]; //@"lastMessage"			
    NSTimeInterval seconds = [[NSDate date] timeIntervalSinceDate:message.updatedAt];
    labelElapsed.text = TimeElapsed(seconds);
    int counter = [message[PF_MESSAGES_COUNTER] intValue]; //@"counter"
